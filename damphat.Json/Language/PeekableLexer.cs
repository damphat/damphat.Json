...
namespace damphat.Json.Language
{
    public class PeekableLexer
    {
        ...
        {
            if (delta < Min || delta > Max) throw new ArgumentOutOfRangeException(nameof(delta));

            var c = Cursor + delta;
            if (c < 0) throw new ArgumentOutOfRangeException(nameof(delta), "cursor + delta < 0");

            if (c < readCount - buffer.Length)
                throw new ArgumentOutOfRangeException(nameof(delta), "cursor + delta < readCount - bufferLength");

            while (readCount <= c) Next();
        ...
        public override string ToString()
        {
            ...
            if (Cursor >= readCount) sb.Append('(').Append(Cursor).Append(')');
